...
for i in range(tests):
    ...
    os.system('python merge_sort.py <input.txt >merge_sort_output.txt')
    
    with open('model_output.txt') as f:
        model_output = f.read()

    with open('merge_sort_output.txt') as f:
        merge_sort_output = f.read()

    if model_output != merge_sort_output:
        NOT_EQUAL = True
