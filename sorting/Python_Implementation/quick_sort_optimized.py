...
# Before calling partition, we put at the end of the current list the middle value between the first, the last and the median
  By doing so the algorithm becomes deterministic (with respect to calling randint() to choose a random element)
...
def quick_sort(A, left, right):
    if left < right:
        ...
        # but as mentioned above, then the algorithm is not deterministic anymore
        # Swap
        temp = A[right] 
        A[right] = A[index]
        A[index] = temp
