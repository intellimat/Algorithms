...
for i in range(tests):
    ...
    os.system('python quick_sort_optimized.py <input.txt >quick_sort_optimized_output.txt')
    
    with open('model_output.txt') as f:
        model_output = f.read()

    with open('quick_sort_optimized_output.txt') as f:
        quick_sort_optimized_output = f.read()

    if model_output != quick_sort_optimized_output:
        NOT_EQUAL = True
