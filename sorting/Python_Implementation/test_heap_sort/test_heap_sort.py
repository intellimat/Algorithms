...
for i in range(tests):
    ...
    os.system('python heap_sort.py <input.txt >heap_sort_output.txt')
    
    with open('model_output.txt') as f:
        model_output = f.read()

    with open('heap_sort_output.txt') as f:
        heap_sort_output = f.read()

    if model_output != heap_sort_output:
        NOT_EQUAL = True
